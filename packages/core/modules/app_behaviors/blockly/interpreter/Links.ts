...
export default class Links extends AbstractInterpreter {
  ...
  get context() {
    const { activeScenario } = useModule("app_components") as AppComponentsModule;
   ...
   */
  _getLinks(id) {
    let { el: root } = useModule("app_renderer") as AppRendererModule;
    root = unref(root);
  ...
  _setupListener(type, id, event, callback) {
    if (type === "BehaviorTrigger") {
      ...
      const { getComponent } = useModule("app_components") as AppComponentsModule;
      const { getComponentElement } = useModule("app_renderer") as AppRendererModule;
  ...
  _setupAutoHighlight(trigger, from, to) {
    ...
    const { linksAutoHighlightClass } = useModule("app_components") as AppComponentsModule;
    const { addCuepoint } = useModule("media_cuepoints") as MediaCuepointsModule;
